      ...
      <div className="bg-gradient-to-br from-slate-50 via-indigo-50/30 to-purple-50/20">
        <div className="max-w-[1600px] mx-auto p-4 md:p-8 lg:p-10 space-y-5 md:space-y-8">
          ...
          <div className="space-y-3 md:space-y-4">
            ...
            <Skeleton className="h-24 md:h-32 w-full rounded-2xl" />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-gradient-to-br from-slate-50 via-indigo-50/30 to-purple-50/20">
      <div className="max-w-[1600px] mx-auto p-4 md:p-8 lg:p-10 space-y-5 md:space-y-8 animate-in fade-in duration-700">
        ...
        {/* ── Hero ─────────────────────────────────────── */}
        <div className={cn("relative overflow-hidden rounded-2xl md:rounded-3xl bg-gradient-to-br p-5 md:p-10 shadow-2xl", heroGradient)}>
          ...
          <div className="relative z-10 space-y-3 md:space-y-4">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start gap-4 md:gap-6">
              <div className="space-y-2 md:space-y-3 flex-1 min-w-0">
                <div className="flex items-center gap-2 md:gap-3">
                  ...
                  <Badge className={cn(
                    "backdrop-blur-sm text-white border-white/30 text-[10px] md:text-xs",
                    topico.resolvido ? "bg-emerald-500/80" : "bg-white/20"
                  )}>
                    {topico.resolvido ? 'Resolvido' : 'Em Aberto'}
                  </Badge>
                </div>
                <h1 className="text-xl md:text-3xl lg:text-4xl font-bold text-white leading-tight break-words">
                  {topico.titulo}
                </h1>
                ...
                {(topico.contexto || topico.tarefa || topico.agendamento) && (
                  <div className="flex flex-wrap gap-1.5 md:gap-2">
                    {topico.contexto && (
                      <span className="text-[10px] md:text-xs bg-white/15 text-white border border-white/25 px-2 md:px-3 py-0.5 md:py-1 rounded-full font-medium truncate max-w-[220px]">
            ...
            {/* Quick Stats */}
            <div className="grid grid-cols-2 gap-2.5 md:gap-4 mt-2">
              <div className="bg-white/10 backdrop-blur-sm rounded-xl md:rounded-2xl p-3 md:p-4 border border-white/20">
                <p className="text-white/70 text-[10px] md:text-xs font-medium mb-0.5 md:mb-1">Respostas</p>
                <p className="text-white text-xl md:text-2xl font-bold">{topico.respostas.length}</p>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-xl md:rounded-2xl p-3 md:p-4 border border-white/20">
                <p className="text-white/70 text-[10px] md:text-xs font-medium mb-0.5 md:mb-1">Status</p>
                <p className={cn("text-xs md:text-sm font-bold", topico.resolvido ? "text-emerald-200" : "text-amber-200")}>
                  {topico.resolvido ? '✓ Resolvido' : '⏳ Em aberto'}
                </p>
              </div>
            </div>
        ...
        <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm overflow-hidden">
          ...
          <CardHeader className="pb-3 md:pb-4 pt-4 md:pt-5 px-4 md:px-6">
            <div className="flex items-center gap-2.5 md:gap-3">
              <Avatar className="h-9 w-9 md:h-11 md:w-11 ring-2 ring-slate-100 shadow-sm">
                <AvatarImage src={topico.autor.avatar || ''} />
              ...
              <div>
                <p className="font-bold text-slate-900 text-sm md:text-base">{topico.autor.nome}</p>
        ...
        <section className="space-y-3 md:space-y-4">
          <div className="flex items-center gap-2.5 md:gap-3">
            ...
            <div>
              <h2 className="text-base md:text-xl font-bold text-slate-900">Respostas</h2>
              ...
              <CardContent className="py-12 md:py-16">
                <div className="flex flex-col items-center justify-center text-center max-w-md mx-auto space-y-4 md:space-y-5">
                  <div className="relative">
                    ...
                    <div className="relative p-4 md:p-5 bg-gradient-to-br from-indigo-100 to-purple-100 rounded-2xl md:rounded-3xl">
                      <MessageSquare className="h-10 w-10 md:h-12 md:w-12 text-indigo-600" />
                    </div>
                  </div>
                  <div className="space-y-1.5 md:space-y-2">
                    <h3 className="text-lg md:text-xl font-bold text-slate-900">Nenhuma resposta ainda</h3>
              ...
              {topico.respostas.map((resposta, index) => (
                ...
                >
                  ...
                  <CardContent className="p-3.5 md:p-6">
                    {/* Badge solução */}
                    {resposta.isSolucao && (
                      <div className="flex items-center gap-1.5 md:gap-2 mb-3 md:mb-4 px-2 md:px-3 py-1 md:py-1.5 bg-emerald-100 text-emerald-700 rounded-lg md:rounded-xl w-fit border border-emerald-200">
                        <Crown size={12} className="text-emerald-600 md:hidden" />
                        <Crown size={14} className="text-emerald-600 hidden md:block" />
                        <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider">Solução</span>
                      </div>
                    )}

                    <div className="flex items-start gap-2.5 md:gap-3 mb-3 md:mb-4">
                      <Avatar className={cn(
                        "h-8 w-8 md:h-10 md:w-10 ring-2 shadow-sm shrink-0",
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 md:gap-2 flex-wrap">
                          ...
                          {resposta.autor.isMe && (
                            <Badge className="text-[8px] md:text-[10px] bg-slate-100 text-slate-500 border border-slate-200 hover:bg-slate-100 uppercase font-bold h-4 px-1.5">
                              Você
                            </Badge>
                          )}
                        </div>
        ...
        <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm overflow-hidden">
          ...
          <CardContent className="p-4 md:p-6 space-y-3 md:space-y-4">
            <div className="flex items-center gap-2.5 md:gap-3 mb-1 md:mb-2">
              <div className="p-1.5 md:p-2 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-lg md:rounded-xl shadow-sm">
                <Send className="h-3.5 w-3.5 md:h-4 md:w-4 text-white" />
              </div>
              <div>
                <h3 className="font-bold text-slate-900 text-sm md:text-base">Sua Resposta</h3>
                <p className="text-[10px] md:text-xs text-slate-500">Seja gentil e colaborativo</p>
              </div>
            </div>

            <form onSubmit={handleSubmitResposta} className="space-y-3 md:space-y-4">
              <Textarea
                value={novaResposta}
                onChange={(e) => setNovaResposta(e.target.value)}
                className="min-h-[100px] md:min-h-[130px] resize-none border-slate-200 focus-visible:ring-indigo-500 text-sm md:text-base"
              ...
              <div className="flex justify-end">
                <Button
                  ...
                  className="w-full sm:w-auto bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white shadow-lg hover:shadow-xl gap-2 min-w-[140px] md:min-w-[160px] font-semibold text-sm md:text-base h-10 md:h-11"
                >
                  {submitting
