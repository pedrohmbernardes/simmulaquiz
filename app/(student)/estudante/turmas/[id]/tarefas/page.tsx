      ...
      <div className="max-w-[1600px] mx-auto p-6 md:p-8 lg:p-10 space-y-8 animate-in fade-in duration-700">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-violet-600 via-purple-600 to-fuchsia-600 p-8 md:p-10 shadow-2xl">
          <div className="absolute inset-0 opacity-10">
            ...
            <div className="absolute bottom-0 left-1/4 w-72 h-72 bg-fuchsia-300 rounded-full blur-3xl" />
          </div>
          ...
          <div className="relative z-10 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4 mt-6">
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
                <p className="text-violet-100 text-xs font-medium mb-1">Pendentes</p>
                <p className="text-white text-2xl font-bold">{pendentes.length}</p>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
                <p className="text-violet-100 text-xs font-medium mb-1">Concluídas</p>
                <p className="text-white text-2xl font-bold">{concluidas.length}</p>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/20">
                <p className="text-violet-100 text-xs font-medium mb-1">Atrasadas</p>
                <p className="text-white text-2xl font-bold">{atrasadas.length}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <TabsList className="grid w-full max-w-md grid-cols-2 mb-6">
            <TabsTrigger value="pendentes" className="flex items-center gap-2">
              ...
              {pendentes.length > 0 && (
                <Badge
                  variant="secondary"
                  className="ml-1 h-5 px-1.5 bg-violet-100 text-violet-700 text-[10px]"
                >
                  {pendentes.length}
                </Badge>
              )}
                  ...
                  >
                    ...
                    <CardContent className="p-5 md:p-6">
                      <div className="flex items-start gap-4">
                        ...
                        <div
                          className={cn(
                            "flex-shrink-0 h-14 w-14 rounded-2xl flex items-center justify-center shadow-lg transition-all duration-300 group-hover:scale-110",
                            tarefa.isAtrasado
                        ...
                        <div className="flex-1 min-w-0 space-y-2">
                          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                            <h3 className="font-bold text-xl text-slate-900 group-hover:text-violet-700 transition-colors line-clamp-1">
                              {tarefa.titulo}
                            </h3>
                            {tarefa.isAtrasado && (
                              <Badge variant="destructive" className="uppercase text-[10px] self-start flex-shrink-0">
                                Atrasado
                              </Badge>
                            )}
                          </div>
                          ...
                          <div className="flex flex-wrap items-center gap-4 text-xs font-medium pt-1">
                            {tarefa.dataEntrega ? (
                              ...
                              >
                                ...
                                {tarefa.isAtrasado ? "Venceu em: " : "Entrega: "}
                                {format(tarefa.dataEntrega, "dd 'de' MMMM", { locale: ptBR })}
                              </span>
                            ) : (
                              <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-50 text-slate-500 border border-slate-200">
                                <Clock size={13} />
                                Sem prazo definido
                              </span>
                            )}
                        ...
                        <ChevronRight className="text-slate-300 group-hover:text-violet-500 transition-colors self-center flex-shrink-0" />
                      </div>
                    </CardContent>
                  </Card>
                </Link>
                  ...
                  >
                    ...
                    <CardContent className="p-5 md:p-6">
                      <div className="flex items-center gap-4">
                        <div className="flex-shrink-0 h-14 w-14 rounded-2xl flex items-center justify-center shadow-lg bg-gradient-to-br from-emerald-500 to-teal-500 text-white transition-all duration-300 group-hover:scale-110">
                          <CheckCircle2 size={24} />
                        </div>

                        <div className="flex-1 min-w-0 space-y-1.5">
                          <div className="flex flex-wrap items-center gap-2">
                            <h3 className="font-bold text-slate-700 group-hover:text-emerald-700 transition-colors text-lg">
                              {tarefa.titulo}
                            </h3>
                            {tarefa.nota !== null && tarefa.nota !== undefined && (
                              <Badge className="bg-emerald-100 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 font-bold">
                                Nota: {tarefa.nota} / {tarefa.notaMaxima}
                              </Badge>
                            )}
                          </div>
