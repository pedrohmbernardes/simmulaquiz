...
function computeWindow(periodo: Periodo) {
  ...
  if (periodo === "ROLLING_90D") {
    return {
      inicio: addDaysUTC(fim, -90),
      fim,
    };
  }

  if (periodo === "ROLLING_180D") {
    return {
      inicio: addDaysUTC(fim, -180),
      fim,
    };
  }

  if (periodo === "ROLLING_12M") {
    return {
      inicio: addYearsUTC(fim, -1),
      fim,
    };
  }

  // GERAL: janela diária
  // mantém snapshots diários de XP_TOTAL e STREAK
  return {
    inicio: new Date(Date.UTC(1970, 0, 1, 0, 0, 0, 0)),
    fim,
  };
}
...
async function buildXpGanho(
  tx: any,
  inicio: Date,
  fim: Date,
): Promise<RankRow[]> {
  // inclui todos usuários ativos
  // para poder calcular posição fora do top 50
  ...
  for (const r of agrupado) {
    byId.set(r.usuarioId, r._sum.quantidade ?? 0);
  }
...
async function buildXpTotal(tx: any): Promise<RankRow[]> {
  ...
  for (const g of gam) {
    byId.set(g.usuarioId, g.pontos ?? 0);
  }
...
async function buildStreak(tx: any): Promise<RankRow[]> {
  ...
  rows.sort(
    (a, b) =>
      b.valorInt - a.valorInt || b._xp - a._xp || a.usuarioId - b.usuarioId,
  );

  return rows.map(({ usuarioId, valorInt }) => ({
    usuarioId,
    valorInt,
  }));
}
...
async function upsertSnapshotWithEntries(args: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    // Recria entries apenas em modo force
    // ou quando snapshot é novo.
    if (existing) {
...
async function handler(req: NextRequest) {
  ...
  if (!requireCronAuth(req)) {
    console.warn("[cron/ranking-snapshot] Requisição não autorizada", {
      timestamp,
    });
  ...
  if (!rl.success) {
    ...
    console.warn("[cron/ranking-snapshot] Rate limit atingido", {
      timestamp,
      ip,
      durationMs,
    });
  ...
  if (!locked) {
    ...
    console.warn("[cron/ranking-snapshot] IGNORADO - lock ativo", {
      timestamp,
      durationMs,
    });
  ...
  try {
    ...
    if (!parsed.success) {
      ...
      console.warn("[cron/ranking-snapshot] Parâmetros inválidos", {
        timestamp,
        durationMs,
      });
    ...
    if (mode === "SINGLE") {
      ...
      // AUTO:
      // gera tudo que /api/ranking consome
    ...
    const snapshotsCriados = results.filter((r) => !r.skipped).length;

    const snapshotsIgnorados = results.filter((r) => r.skipped).length;

    const totalEntriesCriadas = results.reduce(
      (total, r) => total + r.entriesCreated,
      0,
    );
    ...
    console.log("[cron/ranking-snapshot] SUCESSO", {
      timestamp,
      mode,
      force,

      jobsExecutados: results.length,
    ...
    await releaseLock().catch((error) => {
      console.error("[cron/ranking-snapshot] Falha ao liberar advisory lock", {
        timestamp,
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error("[cron/ranking-snapshot] FALHA", {
      timestamp,
      method: "GET",
      durationMs,
      error: errorMessage,
