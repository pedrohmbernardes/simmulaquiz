...
const perfilUpdateSchema = z.object({
  nome: z.string().trim().min(3, 'Nome muito curto').max(80, 'Nome muito longo').optional(),
  email: z.string().trim().toLowerCase().email('E-mail inválido').max(254).optional(),
  ...
  const temSenhaAtual = typeof data.senhaAtual === 'string' && data.senhaAtual.length > 0;
  const temNovaSenha = typeof data.novaSenha === 'string' && data.novaSenha.length > 0;
...
export async function GET() {
  try {
    ...
    const usuario = await prisma.usuario.findUnique({
      ...
      select: {
        ...
        conquistas: {
          ...
          select: {
            dataConquista: true,
            conquista: { select: { id: true, nome: true, descricao: true, icone: true } }
          }
        }
      }
    });

    if (!usuario) return NextResponse.json({ error: 'Usuário inexistente' }, { status: 404 });
    ...
    console.error("Erro perfil GET:", error instanceof Error ? error.message : String(error));
...
export async function PUT(request: Request) {
  try {
    ...
    const fotoUrlRaw = Object.prototype.hasOwnProperty.call(bodyRaw, 'fotoUrl') ? bodyRaw.fotoUrl : undefined;
    ...
    if (validacao.data.senhaAtual && validacao.data.novaSenha) {
      ...
      if (authRateLimit) {
        ...
        if (!rlPass.success) return NextResponse.json({ error: 'Muitas tentativas de senha. Aguarde 30 min.' }, { status: 429 });
      }

      const usuarioSenha = await prisma.usuario.findUnique({ where: { id: userId }, select: { senhaHash: true } });
      if (!usuarioSenha) return NextResponse.json({ error: 'Sessão inválida.' }, { status: 401 });

      const ok = await bcrypt.compare(validacao.data.senhaAtual, usuarioSenha.senhaHash);
      ...
      await prisma.usuario.update({
        where: { id: userId },
        data: { senhaHash: novaHash, tokenVersion: { increment: 1 }, mudancaSenhaObrigatoria: false }
      });
      tokenVersionAtual += 1;
    ...
    if (validacao.data.nome !== undefined) dadosParaAtualizar.nome = validacao.data.nome;
    if (validacao.data.dataNascimento !== undefined) dadosParaAtualizar.dataNascimento = validacao.data.dataNascimento;
    if (validacao.data.fotoUrl !== undefined) dadosParaAtualizar.fotoUrl = validacao.data.fotoUrl;
    ...
    if (validacao.data.email && validacao.data.email !== sessao.email) {
      ...
      const emailExiste = await prisma.usuario.findUnique({ where: { email: novoEmail }, select: { id: true } });
      if (emailExiste && emailExiste.id !== userId) return NextResponse.json({ error: 'E-mail em uso.' }, { status: 400 });
      ...
      if (!validacao.data.codigoEmail) {
        ...
        const res = NextResponse.json({ step: 'EMAIL_VERIFICATION_REQUIRED', message: `Código enviado para ${novoEmail}` });
        res.cookies.set('pending_email_change', novoEmail, {
          httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', path: '/api/estudante/perfil', maxAge: 600,
        });
        return res;
      }
      ...
      if (otpRateLimit) {
        ...
        if (!rlOtp.success) return NextResponse.json({ error: 'Muitas tentativas de código. Aguarde 10 min.' }, { status: 429 });
      }

      const cookieStore = await cookies();
      const pendingEmail = cookieStore.get('pending_email_change')?.value;
      if (!pendingEmail || pendingEmail !== novoEmail) return NextResponse.json({ error: 'Fluxo inválido. Solicite novo código.' }, { status: 400 });

      const usuarioOTP = await prisma.usuario.findUnique({ where: { id: userId }, select: { tokenVerificacao: true, tokenExpiraEm: true } });
      
      const expirou = !usuarioOTP?.tokenExpiraEm || new Date() > usuarioOTP.tokenExpiraEm;
      // Comparação Timing Safe
      const codigoOk = !expirou && safeCompare(usuarioOTP?.tokenVerificacao, validacao.data.codigoEmail);

      if (!codigoOk) return NextResponse.json({ error: 'Código inválido ou expirado.' }, { status: 400 });
    ...
    if (Object.keys(dadosParaAtualizar).length > 0) {
      await prisma.usuario.update({ where: { id: userId }, data: dadosParaAtualizar });
    ...
    if (dadosParaAtualizar.email) {
      ...
      res.cookies.set('pending_email_change', '', { maxAge: 0, path: '/api/estudante/perfil' });
      return res;
    }
    ...
    console.error("Erro perfil PUT:", error instanceof Error ? error.message : String(error));
    return NextResponse.json({ error: 'Erro interno.' }, { status: 500 });
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    const csrfHeader = request.headers.get('x-csrf-token');
    const csrfOk = await verifyCSRFToken(csrfHeader);
    if (!csrfOk) return NextResponse.json({ error: 'CSRF inválido.' }, { status: 403 });
    ...
    if (!body.codigo) {
      ...
      await prisma.usuario.update({ where: { id: userId }, data: { tokenVerificacao: otp, tokenExpiraEm: expiracao } });
      await enviarCodigoExclusaoConta(sessao.email, otp, sessao.name);
      return NextResponse.json({ step: 'CONFIRMATION_REQUIRED', message: 'Código enviado.' });
    }

    // Validar Código (Protegido por OTP Rate Limit)
    if (otpRateLimit) {
      const rlOtp = await otpRateLimit.limit(`otp_del:${userId}`);
      if (!rlOtp.success) return NextResponse.json({ error: 'Muitas tentativas. Aguarde 10 min.' }, { status: 429 });
    }

    const usuario = await prisma.usuario.findUnique({ where: { id: userId }, select: { id: true, tokenVerificacao: true, tokenExpiraEm: true } });
    const expirou = !usuario?.tokenExpiraEm || new Date() > usuario.tokenExpiraEm;
    // Comparação Timing Safe
    const codigoOk = !expirou && safeCompare(usuario?.tokenVerificacao, body.codigo);

    if (!codigoOk) return NextResponse.json({ error: 'Código inválido ou expirado.' }, { status: 400 });

    await registrarLog({ acao: 'USUARIO_EXCLUIR' as any, usuarioId: userId, detalhes: { ip } });
    ...
    console.error("Erro perfil DELETE:", error instanceof Error ? error.message : String(error));
