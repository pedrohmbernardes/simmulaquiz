...
import { SECRET, TEMPO_EXPIRACAO, MAX_AGE_SECONDS, verifyJWT, type SessionPayload } from '@/lib/auth-edge';
...
function isSameOrigin(request: NextRequest): boolean {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (originHost.startsWith('localhost') || originHost.startsWith('127.0.0.1') || originHost.startsWith('[::1]')) {
        return true;
      }
...
async function verifyCSRFEdge(request: NextRequest): Promise<boolean> {
  ...
  if (!secretKey) {
      console.error('[MIDDLEWARE-DEBUG] ❌ CSRF FALHOU: Secret Key não definida.');
      return false;
  }
  ...
  if (!headerToken || !cookieToken) {
      console.error(`[MIDDLEWARE-DEBUG] ❌ CSRF FALHOU: Dados faltando. Header: ${!!headerToken}, Cookie: ${!!cookieToken}`);
      // Se estiver no browser, mostre os cookies que chegaram para entender o bloqueio
      console.log('[MIDDLEWARE-DEBUG] Cookies recebidos:', request.cookies.getAll().map(c => c.name));
      return false;
  }

  if (headerToken !== cookieToken) {
      console.error('[MIDDLEWARE-DEBUG] ❌ CSRF FALHOU: Header diferente do Cookie (Double Submit Mismatch).');
      return false;
  }

  try {
    const key = new TextEncoder().encode(secretKey);
    const { payload } = await jwtVerify(headerToken, key, { algorithms: ['HS256'] });
    if ((payload as any).csrf !== true) {
        console.error('[MIDDLEWARE-DEBUG] ❌ CSRF FALHOU: Payload do token inválido.');
        return false;
    }
    return true;
  } catch (e) {
    console.error('[MIDDLEWARE-DEBUG] ❌ CSRF FALHOU: Erro na verificação JWT.', e);
    return false;
  }
}
...
export async function middleware(request: NextRequest) {
  ...
  if (isMutatingMethod(method)) {
    ...
    if (contentLength) {
      ...
      if (Number.isFinite(n) && n > 5 * 1024 * 1024) {
        return jsonResponse({ error: 'Requisição muito grande. Máximo: 5MB' }, { status: 413 });
      }
    }
  ...
  if (path === '/login' || publicPrefixes.some((p) => path === p || path.startsWith(p + '/'))) {
  ...
  const isCriticalPath = criticalPrefixes.some((p) => path === p || path.startsWith(p + '/'));
  ...
  if (isCriticalPath && isMutatingMethod(method)) {
    ...
    if (!ok) {
      ...
      return jsonResponse(
        { error: 'Sessão inválida ou token de segurança expirado. Recarregue a página.' },
        { status: 403 }
      );
    }
  }
  ...
  if (isProtected) {
    ...
    if (!user) {
      ...
      if (path.startsWith('/api/')) {
        console.error(`[MIDDLEWARE-DEBUG] ⛔ 401 Não Autenticado em API: ${path}. Cookie recebido: ${!!sessionCookie}`);
    ...
    if (user.mudancaSenhaObrigatoria && !path.startsWith('/nova-senha') && !path.startsWith('/api/auth')) {
      return redirectResponse(new URL('/nova-senha', request.url));
    }
    ...
    const isAdminPath = 
        ...
        path === '/api/ai/generate' || path.startsWith('/api/ai/generate/');

    if (isAdminPath && user.role !== 'PROFESSOR' && user.role !== 'SUPER_ADMIN') {
      if (path.startsWith('/api/')) {
